...
#               rott = wl[, "rott"]))
# for (speciality_name in speciality) {
#   chosen_df <- get(paste0("df_", speciality_name))
ts.data_52<- ts(wl_52%>%dplyr::select(wm52))
ARIMA_52<- auto.arima(ts.data_52, seasonal = F, stepwise = F, approximation = F, trace = T, lambda = 0)
# png(here("plots/forecast_arima/", paste0(speciality_name, "_wl52.png")))
fc_wl <- forecast::forecast(ARIMA_52, biasadj = F, h = 365)
autoplot(fc_wl)
# dev.off()

ts.data_wl<- ts(wl%>%dplyr::select(patients))
ARIMA_wl<- auto.arima(ts.data_wl, seasonal = F, stepwise = F, approximation = F, trace = T, lambda=0)
