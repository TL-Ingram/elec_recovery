...
AND removed_date_dt BETWEEN CAST('",
  ...
  to_write <- cbind(adms_daily, dta_daily, rott_daily) %>%
    select(1,2,4,6) %>%
    mutate(id = 1:nrow(adms_daily)) %>%
    mutate(speciality = specialty) %>%
    select(-id)

  write_csv(to_write, paste0(here("csv", "parameters", specialty), ".csv" ))
}
getwd()
