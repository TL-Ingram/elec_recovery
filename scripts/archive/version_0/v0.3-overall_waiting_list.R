...
for (specialty_name in specialty)
  ...
  q_params <-
    paste0("SELECT * FROM elective_recovery_params where creator = 'system' and time_period = 'current'")
  parameters <-
  ...
  q_times_to_dta <- paste0("SELECT * FROM elective_recovery_times_to_dta where time_period = 'current'")
  times_to_dta <-
    hsql(q_times_to_dta, db = "nhs_datascience", server = "wwldevsqlfrm1")
  print(paste0("Times to DTA have been loaded."))
  
  times_to_dta_spec <- times_to_dta %>%
    group_by(priority, days) %>%
    mutate(mean = mean(prob)) %>%
    distinct(mean) %>%
    rename("prob" = mean)
  
  q_times_to_planned <- paste0("SELECT * FROM elective_recovery_times_to_planned where time_period = 'current'")
  times_to_planned <-
    hsql(q_times_to_planned, db = "nhs_datascience", server = "wwldevsqlfrm1")
  print(paste0("Times to planned have been loaded."))
  
  times_to_planned_spec <- times_to_planned %>%
    group_by(months) %>%
    mutate(mean = mean(prob)) %>%
    distinct(mean)
  ...
  print(paste0("Waiting list has been formatted for entire waiting list"))
  ...
  capacity_priority_splits <-
    ...
    mutate(value = ifelse(is.na(sum), 0, sum))#P2,P3,P4
  ...
  demand_priority_splits <-
    ...
    full_join(data.frame(priority = c("Priority 2","Priority 3", "Priority 4", "Unknown", "Planned"))) %>%
    mutate(value = ifelse(is.na(sum), 0, sum))
  ...
  write_rds(res.sum, paste0(here("rds", "forecast_horizon_FULL_WL.rds")))
  write_rds(results, paste0(here("rds", "clearance_times_FULL_WL.rds")))
